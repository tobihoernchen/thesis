...
from .custom_blocks import MatrixPositionEmbedder, TransformerDecoderBlock, TransformerEncoderBlock, FourierFeatureEmbedder
...
class AttnFE(nn.Module):
    def __init__(
        ...
        self.embedd_main = self.get_embedder(
            n_features + position_embedd_dim * 7 + 2*7*ff_embedd_dim, embed_dim, activation
        )
        if self.with_agvs:
        ...
        if self.with_stations:
            self.embedd_station = self.get_embedder(
                n_features + position_embedd_dim + 2*ff_embedd_dim, embed_dim, activation
            )
